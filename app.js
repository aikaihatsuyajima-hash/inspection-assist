...
function downloadHistoryExcel() {
  ...
  const now = new Date(); const pad = (value) => String(value).padStart(2, "0"); const fileTimestamp = `${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
